...
Let’s start by creating a new `RemarkCommand` class in the `src/main/java/nustracker/logic/command` directory.
...
Create a `RemarkCommandParser` class in the `nustracker.logic.parser` package. The class must extend the `Parser` interface.
...
Create a new `Remark` in `nustracker.model.student`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Simply add the following to [`nustracker.ui.StudentCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-0c6b6abcfac8c205e075294f25e851fe).
